...
namespace IBox.Schedule.Library.HandleShrinkLogDB
{
    public class HandleShrinkLog : IHandleShrinkLog
    {
        ...
        public void RunShrinkLogDatabase(IJobExecutionContext context)
        {
            ...
            try
            {
                Log.Information($"Start Shrink log database with listCategory:{listCategory}, jobKey: {jobKey}, scheduleId: {scheduleId}, deploymentType: {deploymentType}");
                ...
                switch (deploymentType)
                {
                    ...
                    default:
                        Log.Error($"Not found DeploymentType: {deploymentType} in config.", "AppLogs");
                        break;
                }
            }
            catch (Exception ex)
            {
                throw new IboxLog($"RunShrinkLogDatabase An Unexpected Error Has Occurred: {ex.Message}", "AppLogs", ex);
            }
        }
        ...
        private D_DatabaseConnection DatabaseConnectionInfo()
        {
            ...
            databaseConfigShrinkLog.Password = _encryption.Decrypt(databaseConfigShrinkLog.Password ?? "");
