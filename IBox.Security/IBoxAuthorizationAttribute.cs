...
{
    public class IBoxAuthorizationAttribute : Attribute, IAsyncAuthorizationFilter
    {
...
#pragma warning restore CS1998 // Async method lacks 'await' operators and will run synchronously
        {
            ...
            string role = jwt.Payload.FirstOrDefault(ptr => ptr.Key == "Role").Value.ToString() ?? string.Empty;
            string userId = jwt.Payload.FirstOrDefault(ptr => ptr.Key == "UserId").Value.ToString() ?? string.Empty;
            string permissionsKey = jwt.Payload.FirstOrDefault(ptr => ptr.Key == "PermissionsKey").Value.ToString() ?? string.Empty;
            ...
            headers[RequestHeaderKey.Role.ToString()] = role;
            headers[RequestHeaderKey.UserId.ToString()] = userId;
            headers[RequestHeaderKey.PermissionsKey.ToString()] = permissionsKey;
