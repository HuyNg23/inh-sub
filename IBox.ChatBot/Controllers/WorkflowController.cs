...
namespace IBox.ChatBot.Controllers
{
    ...
    public class WorkflowController : ControllerBase
    {
        ...
        public ResponseForm<dynamic> DeployWF(string id)
        {
            ...
            return new ResponseForm<dynamic>(() => _executeWF.SetTenantContext(_tenantContext, tenantID).Deploy(id, this.Request, ViewDeployServiceType.ChatBot));
        }
        ...
        {
            var tenantID = Request.Headers.FirstOrDefault(ptr => ptr.Key == RequestHeaderKey.Tenant.ToString()).Value.ToString() ?? string.Empty;
            return new ResponseForm<dynamic>(() => _executeWF.SetTenantContext(_tenantContext, tenantID).Recovery(id, this.Request, ViewDeployServiceType.ChatBot));
        }
        ...
        {
            var tenantID = Request.Headers.FirstOrDefault(ptr => ptr.Key == RequestHeaderKey.Tenant.ToString()).Value.ToString() ?? string.Empty;
            return new ResponseForm<dynamic>(() => _executeWF.SetTenantContext(_tenantContext, tenantID).ViewWFDeploy(this.Request, ViewDeployServiceType.ChatBot));
        }
